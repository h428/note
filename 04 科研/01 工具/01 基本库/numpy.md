...
一维的数组可以进行索引、切片和迭代操作的，就像列表和其他 Python 序列类型一样。
...
- 如果只放置一个参数，没有冒号，如 `[2]`，则取出对应元素，对于 n 维数组则是维度 - 1 的子数组
- 如果为 `[2:]`，表示从该索引开始以后的所有项都将被提取
- 如果使用了两个参数，如 `[2:7]`，那么则提取两个索引（不包括 stop 索引）之间的项，即默认步长为 1
- 选择某行或某列的所有元素，可以使用 ... 或者 : 表示所有元素
- 可以使用 `step = -1` 来反序指定向量，例如 `a[ : :-1]`
